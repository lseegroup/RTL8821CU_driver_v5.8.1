...
u8 WapiGetEntryForCamWrite(_adapter *padapter, u8 *pMacAddr, u8 KID, BOOLEAN IsMsk)
{
	...
	{
		if (pWapiInfo->wapiCamEntry[i].IsUsed && (_rtw_memcmp(pMacAddr, pWapiInfo->wapiCamEntry[i].PeerMacAddr, ETH_ALEN) == _TRUE) && pWapiInfo->wapiCamEntry[i].keyidx == KID && pWapiInfo->wapiCamEntry[i].type == IsMsk)
		{
...
u8 WapiGetEntryForCamClear(_adapter *padapter, u8 *pPeerMac, u8 keyid, u8 IsMsk)
{
	...
	for (i = 0; i < WAPI_CAM_ENTRY_NUM; i++)
	{
		if (pWapiInfo->wapiCamEntry[i].IsUsed && (_rtw_memcmp(pPeerMac, pWapiInfo->wapiCamEntry[i].PeerMacAddr, ETH_ALEN) == _TRUE) && pWapiInfo->wapiCamEntry[i].keyidx == keyid && pWapiInfo->wapiCamEntry[i].type == IsMsk)
		{
